...
static void cdc_ecm_client_task(void *arg)
{
    ...
    // Grab a stable snapshot of the driver object under critical
    ...
    if (cdc_ecm_obj == NULL)
    {
        ESP_LOGW(TAG, "cdc_ecm_client_task: driver object is NULL, exiting");
        vTaskDelete(NULL);
    }

    usb_host_client_handle_t client = cdc_ecm_obj->cdc_ecm_client_hdl;
    if (client == NULL)
    {
        ESP_LOGW(TAG, "cdc_ecm_client_task: client handle is NULL, exiting");
        vTaskDelete(NULL);
    }
    ...
    while (1)
    {
        ...
        if (err != ESP_OK)
        {
            ESP_LOGW(TAG, "usb_host_client_handle_events returned %s", esp_err_to_name(err));
            // When the host is being torn down, you typically see ESP_ERR_INVALID_STATE
            break;
        }
    ...
    esp_err_t derr = usb_host_client_deregister(client);
    if (derr != ESP_OK)
    {
        ESP_LOGW(TAG, "usb_host_client_deregister failed: %s", esp_err_to_name(derr));
    }

    if (cdc_ecm_obj->event_group)
    {
...
static esp_err_t cdc_ecm_start(cdc_dev_t *cdc_dev, cdc_ecm_host_dev_callback_t event_cb, cdc_ecm_data_callback_t in_cb, void *user_arg)
{
    ...
    bool data_intf_claimed = false;
    bool notif_intf_claimed = false;
    assert(cdc_dev);
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Could not claim data interface: %s", esp_err_to_name(ret));
        goto err;
    }
    data_intf_claimed = true;

    // Start polling BULK IN, if present
    if (cdc_dev->data.in_xfer)
    {
        ESP_LOGD(TAG, "Submitting poll for BULK IN transfer (start)");
        ret = usb_host_transfer_submit(cdc_dev->data.in_xfer);
        if (ret != ESP_OK)
        {
            ESP_LOGE(TAG, "Failed to submit BULK IN transfer: %s", esp_err_to_name(ret));
            goto err;
        }
    }

    // Optionally prime BULK OUT transfer
    if (cdc_dev->data.out_xfer)
    {
        ...
        if (ret != ESP_OK)
        {
            ESP_LOGE(TAG, "Failed to submit BULK OUT transfer: %s", esp_err_to_name(ret));
            goto err;
        }
    }
    else
    {
        ESP_LOGW(TAG, "No OUT transfer available to submit!");
    }

    // Notification interface (if supported)
    if (cdc_dev->notif.xfer)
    {
        if (cdc_dev->notif.intf_desc != cdc_dev->data.intf_desc)
        {
            ...
            if (ret != ESP_OK)
            {
                ESP_LOGE(TAG, "Could not claim notification interface: %s", esp_err_to_name(ret));
                goto err;
            }
            notif_intf_claimed = true;
        }

        ESP_LOGD(TAG, "Submitting poll for INTR IN transfer (start)");
        ret = usb_host_transfer_submit(cdc_dev->notif.xfer);
        if (ret != ESP_OK)
        {
            ESP_LOGE(TAG, "Failed to submit INTR IN transfer: %s", esp_err_to_name(ret));
            goto err;
        }
    }
...
err:
    // We *haven't* inserted into the list yet, so just unwind claims.
    if (notif_intf_claimed)
    {
...
esp_err_t cdc_ecm_host_data_tx_blocking(cdc_ecm_dev_hdl_t cdc_hdl, const uint8_t *data, size_t data_len, uint32_t timeout_ms)
{
    ...
    if (ret != ESP_OK)
    {
        ...
        {
            ESP_LOGW(TAG, "TX: host/device in invalid state, marking link down");
            // Mark the device as disconnected so higher layers stop sending
            cdc_dev->connect_status = false;
        }
        else
        {
            ESP_LOGE(TAG, "Failed to submit BULK OUT transfer: %s", esp_err_to_name(ret));
    ...
    {
        ESP_LOGE(TAG, "Bulk OUT transfer error, status=%d", cdc_dev->data.out_xfer->status);
    ...
    {
        ESP_LOGE(TAG, "Incorrect number of bytes transferred: %d != %d",
                 (int)cdc_dev->data.out_xfer->actual_num_bytes, (int)data_len);
        ret = ESP_ERR_INVALID_RESPONSE;
        goto unblock;
    }
    ret = ESP_OK;
...
static void usb_lib_task(void *arg)
{
    ...
    s_usb_lib_task_handle = NULL; // task is about to return / be deleted
    vTaskDelete(NULL);
}
...
static void cdc_ecm_teardown(cdc_ecm_params_t *params)
{
    // Stop ETH / destroy netif if it exists
    if (usb_netif)
    {
        esp_event_post(ETH_EVENT, ETHERNET_EVENT_STOP, &usb_netif,
                       sizeof(esp_netif_t *), portMAX_DELAY);
        esp_netif_action_stop(usb_netif, 0, 0, 0);
        esp_netif_destroy(usb_netif);
        usb_netif = NULL;
    }

    // Close CDC device + uninstall CDC-ECM host
    if (cdc_dev)
    {
    ...
    // esp_err_t uret = cdc_ecm_host_uninstall();
    // if (uret != ESP_OK && uret != ESP_ERR_INVALID_STATE)
    // {
    //     ESP_LOGW(TAG, "cdc_ecm_host_uninstall failed: %s", esp_err_to_name(uret));
    // }
...
static void cdc_ecm_task(void *arg)
{
    ...
    {
        ESP_LOGD(TAG, "USB Host already installed, skipping install");
    }

    // Create the USB library event task once too
    if (s_usb_lib_task_handle == NULL)
    {
        BaseType_t task_created = xTaskCreate(
            usb_lib_task,
            "usb_lib",
            4096, // your existing stack size
            NULL,
            CDC_ECM_USB_HOST_PRIORITY,
            &s_usb_lib_task_handle);
        assert(task_created == pdTRUE);
    }

    esp_err_t cerr = cdc_ecm_host_install(NULL);
    if (cerr == ESP_ERR_INVALID_STATE)
    {
        ESP_LOGW(TAG, "CDC-ECM host already installed, continuing");
    }
    else
    {
    ...
    while (true)
    {
        ...
        int attempts = 0;
        const int max_attempts = 10; // ~10 seconds of trying

        while (err != ESP_OK && !s_stop_requested && attempts < max_attempts)
        {
            attempts++;
            ESP_LOGD(TAG, "Trying to open USB device (attempt %d)...", attempts);
            ...
            if (err != ESP_OK)
            {
                // ESP_LOGD(TAG, "cdc_ecm_host_open failed: %s", esp_err_to_name(err));
                if (err == ESP_ERR_INVALID_STATE)
                {
                    ESP_LOGW(TAG, "cdc_ecm_host_open returned INVALID_STATE, treating as fatal");
                    s_stop_requested = true;
                    break;
                }
        ...
        // If we hit the fatal flag OR we just never managed to open the device, bail out
        if (err != ESP_OK)
        {
            ESP_LOGW(TAG, "CDC-ECM: giving up on USB device (err=%s)",
                     esp_err_to_name(err));
            cdc_ecm_teardown(params);
        ...
        // Decide whether to exit or loop
        if (s_stop_requested)
        {
            ESP_LOGI(TAG, "CDC-ECM: stopping task (stop_requested=%d)",
                     (int)s_stop_requested);
            goto fatal_shutdown;
        }
...
fatal_shutdown:
    // ---- cleanup section at bottom of function ----
    ESP_LOGI(TAG, "cdc_ecm_task exiting, cleaning up");
    // Final shutdown: uninstall CDC-ECM host driver
    esp_err_t uret = cdc_ecm_host_uninstall();
    if (uret != ESP_OK && uret != ESP_ERR_INVALID_STATE && uret != ESP_ERR_NOT_FINISHED)
    {
        ESP_LOGW(TAG, "cdc_ecm_host_uninstall (final) failed: %s", esp_err_to_name(uret));
    }
...
void cdc_ecm_init(cdc_ecm_params_t *cdc_ecm_params)
{
    ...
    if (s_cdc_ecm_task_handle != NULL)
    {
        ESP_LOGW(TAG, "cdc_ecm_init: CDC-ECM task already running");
        return;
    }
    if (device_disconnected_sem == NULL)
    {
